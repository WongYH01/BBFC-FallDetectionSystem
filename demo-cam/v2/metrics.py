"""Measurement mode for v2 — per-frame metrics and an end-of-run summary.
...
v2 adds the ensemble decision to every frame row (`p_fall`, the rolling mean,
the window count and the alarm bit) plus a `fall_detection` block in the
summary, so a run says not just how fast the camera was processed but what the
detector decided. The provenance block records which pose backend and which
checkpoints produced it -- a run on the pt pose model and a run on the ONNX one
are not comparable.
...
CSV_FIELDS = [
    ...
    "p_fall", "p_fall_rolling", "n_windows", "alarm",
    "warmup",
]
...
class Run:
    ...
    def add(self, result, t_done: float, fall_state: dict | None = None) -> None:
        """Score one frame. Called once per frame while the run is active.

        `fall_state` is the last ensemble state from `fallcore.stream` (the
        detector keeps it); it is the same dict on frames between window
        scores, which is correct -- the decision did not change on those.
        """
        i = len(self.rows)
        ...
        self.rows.append({
            ...
            "n_kpt_visible": n_vis,
            "n_kpt_total": n_total,
            # "" until the ring buffer fills and the first window is scored.
            "p_fall": (_r(state.get("p_fall")) if state.get("n_windows")
                       else ""),
            "p_fall_rolling": (_r(state.get("rolling_mean"))
                               if state.get("n_windows") else ""),
            "n_windows": int(state.get("n_windows", 0)),
            "alarm": int(bool(state.get("alarm"))) if state else "",
    ...
    def _summarise(self, duration: float, arrivals: int, covered: float) -> dict:
        ...
        # -- ensemble decision ----------------------------------------------
        scored = [r for r in analysed if r["p_fall"] != ""]
        alarm_frames = sum(1 for r in scored if r["alarm"] == 1)
        base["fall_detection"] = {
            "pose_backend": self.provenance.get("pose_backend"),
            "ensemble": self.provenance.get("ensemble"),
            "buffer_len": self.provenance.get("buffer_len"),
            "threshold": self.provenance.get("threshold"),
            "windows_scored": max((r["n_windows"] for r in scored), default=0),
            "mean_window_p": _mean([r["p_fall"] for r in scored]),
            "mean_rolling": _mean([r["p_fall_rolling"] for r in scored]),
            "alarm_frames": alarm_frames,
            "alarm_pct": (round(100 * alarm_frames / len(scored), 1)
                          if scored else None),
            "note": "Alarm is the latched rolling-mean decision, not a "
                    "per-frame probability; alarm_pct is the share of scored "
                    "frames while it was latched. Recall/precision are not "
                    "measurable from a run with no ground truth.",
        }
        return base
...
def provenance_of(model, imgsz: int, pose_backend: str = "unknown",
                  ensemble: list | None = None, buffer: int | None = None,
    ...
    v2 adds which pose backend (onnx or torch) and which classifier
    checkpoints/decision settings were behind the alarm.
    """
    try:
